...
/**
 * Función para procesar mensajes de la cola SQS_CL y almacenar en RDS (Chile).
    ...
    try {
        ...
        for (const record of event.Records) {
            ...
            if (!isValidAppointment(messageBody)) {
                console.error('Mensaje no válido en SQS_CL:', messageBody);
            ...
            console.log('Cita almacenada en RDS (Chile):', appointment);
        }

        // Cerrar la conexión a la base de datos
        await connection.end();
    } catch (error) {
        console.error('Error en el handler de appointment_cl:', error);
